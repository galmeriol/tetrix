...
import capture.IGestureListener;
import capture.Tetrix;
public class Tetris extends Applet implements IGestureListener{

    //
    // STATIC MEMBERS
    //
    static { System.loadLibrary(Core.NATIVE_LIBRARY_NAME); }
    private Tetrix mHandGestureDetector;
    Direction direction = new Direction("Default");
	...
	try {
	    mHandGestureDetector = new Tetrix();
	...
	synchronized(timer) {
	    cur_piece.cut();
	    cur_piece.setX(cur_piece.getX() + -1); // try to move
	    if( ! cur_piece.canPaste())
		cur_piece.setX(cur_piece.getX() - (-1)); // undo move
	    cur_piece.paste();
	}
	game_grid.repaint();
    }
